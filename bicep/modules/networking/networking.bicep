// ─── networking.bicep ────────────────────────────────────────────────────────
// Deploys: VNet with all subnets pre-reserved, NSGs (no rules = Azure defaults),
//          Network Watcher, and optional: Bastion, App Gateway, Azure Firewall,
//          VPN Gateway, NAT Gateway, Private DNS Zones, UDRs (when firewall active)
//
// DESIGN PRINCIPLE: All subnets are ALWAYS provisioned so the address space
// never needs redesign when toggling optional resources. Resources inside subnets
// are conditional based on flags.
//
// DEFAULT MODE: No hardening. NSGs are empty (Azure default-allow within VNet).
//               No forced diagnostic settings. No service endpoints.
// ─────────────────────────────────────────────────────────────────────────────
...
@description('Azure Bastion SKU. Developer = free (no dedicated subnet). Basic/Standard = paid.')
@allowed(['Developer', 'Basic', 'Standard', 'None'])
param bastionSku string = 'Developer'

@description('Deploy Application Gateway with WAF v2. Off by default (~$200-300/month).')
param enableAppGateway bool = false

@description('Deploy Azure Firewall. None = disabled. Standard ~$900/mo. Premium ~$1500/mo.')
@allowed(['None', 'Standard', 'Premium'])
param enableFirewall string = 'None'

@description('Deploy VPN Gateway for Point-to-Site access. Off by default (~$140/month).')
param enableVpnGateway bool = false

@description('Deploy NAT Gateway for outbound internet. Off by default (~$32/month).')
param enableNatGateway bool = false

@description('Deploy DDoS Network Protection. WARNING: ~$2,944/month. Off by default.')
...
var appGwSubnetPrefix           = '10.0.9.0/24'    // AppGW WAF v2 requires /24
var firewallSubnetPrefix        = '10.0.10.0/26'   // AzureFirewallSubnet: /26 minimum
...
// ─── NSGs — Empty by default ──────────────────────────────────────────────────
// No rules = Azure out-of-the-box behaviour: allow all within VNet, deny inbound
// from internet. This is intentional — default mode for benchmark testing.
...
// Note: The firewall private IP is always 10.0.10.4 (first usable IP in /26).
...
// ─── Virtual Network ──────────────────────────────────────────────────────────
// All subnets provisioned. Delegations only on subnets that require it.
// NSG and route table references are null when resources not deployed.
      ...
      [
        ...
        // Reserved subnets — always provisioned, resources are conditional
        {
          ...
          properties: {
            ...
            // AppGW manages its own security; no NSG attached
          }
        }
        {
          ...
          properties: {
            ...
            // Azure Firewall subnet cannot have NSG or route table
          }
        }
        {
          name: 'GatewaySubnet'
          properties: {
            ...
            // VPN Gateway subnet cannot have NSG
          }
        }
      ],
      bastionNeedsSubnet ? [
        {
          ...
          properties: {
            ...
            // Bastion subnet cannot have a route table
          }
        }
      ] : []
    )
...
// ─── Azure Bastion ────────────────────────────────────────────────────────────
// Developer: free, no dedicated subnet, 1 concurrent session
// Basic/Standard: paid, requires AzureBastionSubnet
...
// Off by default. Detection mode (not Prevention) — default out-of-box posture.
...
// ─── Azure Firewall ───────────────────────────────────────────────────────────
// Off by default. Standard ~$900/mo, Premium ~$1,500/mo.
// When enabled, UDRs (defined above) are applied to all workload subnets.
...
// Off by default. VpnGw1 ~$140/month.
// P2S using OpenVPN + Azure AD authentication for easy client connectivity.
...
// ─── Private DNS Zones ────────────────────────────────────────────────────────
// Off by default. Enable when using private endpoints with DNS resolution.
